...
const API_URL = "https://quailio-api.herokuapp.com/api"
...
function displayNetwork(network) {
  const personItems = network.people.map(
    createPersonListItemFactory(network.people),
  )
...
function createPersonListItemFactory(people) {
  return (person) => {
    return `
            <div id=${person.id}>
                <h3>${person.name || "MISSING_NAME"} ${
      person.isGroup ? "[GROUP]" : ""
    }</h3>
                ${
                  person.thumbnailUrl
                    ? `<img src="${person.thumbnailUrl}" style="width:32px;height:32px"></img>`
                    : ""
                }
